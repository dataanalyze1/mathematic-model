...
# 绘制3D柱状图
for i, (x, y, z) in enumerate(zip(x_pos, y_pos, top_features['重要性'].values)):
    # 绘制主柱体
    ax.bar3d(x, y, 0, 0.6, 0.6, z, color=colors[i], shade=True, alpha=0.8)

    # 在柱子上方添加数值标签
    ax.text(x + 0.3, y + 0.3, z + 0.1, f'{z:.2f}',
            ha='center', va='bottom', fontweight='bold', fontsize=9)

    # 在底部添加圆形阴影效果
    theta = np.linspace(0, 2 * np.pi, 30)
    radius = 0.4
    xx = x + 0.3 + radius * np.cos(theta)
    yy = y + 0.3 + radius * np.sin(theta)
    ax.plot(xx, yy, np.zeros_like(xx), color='gray', alpha=0.3)
...
# 设置x轴刻度标签 - 使用简化的特征名称以提高可读性
short_names = [name.split('_')[0] for name in top_features['特征']]
...
# 保存图像
plt.tight_layout()
plt.savefig(r'c:\Users\JIMMYHE\PycharmProjects\统计模型\问卷数据\特征重要性三维柱形图.png', dpi=300,
            bbox_inches='tight')
plt.show()
...
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(r'c:\Users\JIMMYHE\PycharmProjects\统计模型\问卷数据\预测值与真实值时间序列.png', dpi=300,
            bbox_inches='tight')
plt.show()
...
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(r'c:\Users\JIMMYHE\PycharmProjects\统计模型\问卷数据\预测重要性与真实重要性对比.png', dpi=300,
            bbox_inches='tight')
plt.show()
